...
| `onSingleTap`                      | `function`                                                         | Called when the book receives a single tap gesture. Uses native gesture recognition on iOS for improved performance, falls back to custom gesture handler on other platforms. Optional.           |
...
| `contentInserts`                   | `array`                                                            | Inject HTML/JS snippets into the ebook. Each item: `{ targetId, contentId, contentHtml?, contentJavascript?, contentStyle? }`. Runtime updates are synced without reloading the WebView.                                               |
